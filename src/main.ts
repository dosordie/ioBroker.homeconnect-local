  ...
  private buildAutomaticStartOptionValues(device: RunningDevice, programRaw: unknown): Array<{ uid: number; value: unknown }> {
    ...
    if (programOptions) {
      for (const programOption of programOptions) {
        ...
        if (!shouldSendAutomaticStartOption(featureName, rawValue, programOption)) {
          skippedDefaults.push(`${numericUid}=${JSON.stringify(rawValue)}`);
          continue;
        }
    ...
    const skippedList = skippedDefaults.join(", ") || "none";
